...
#         save_path = os.path.join(sanlicy_path, f'epoch_{epoch}.pkl')
...
#             best_model = model
#             best_model_path = os.path.join(sanlicy_path, 'best_model.pkl')
...
#         val_epoch_df.to_csv(os.path.join(sanlicy_path, 'val_epoch_log.csv'), index=False)
...
#     plt.savefig(f'{sanlicy_path}/2cls.pdf', bbox_inches='tight')
#     print(f"Final Accuracy: {acc * 100:.3f}%")

# if __name__ == '__main__':
#     parser = argparse.ArgumentParser(description="FCNA Training and Evaluation")
#     parser.add_argument('--label_filename', type=str, required=True, help="Path to the label CSV file")
#     parser.add_argument('--sanlicy_path', type=str, required=True, help="Directory path to save logs and models")
...
def main(args):
    ...
    external_data_df = pd.read_csv(external_data_path, index_col=0)
    ...
    res_df.to_csv(f'{sanlicy_path}/external_validation_results.csv', index=False)   
    
    plot_confusion_matrix(labe_seq, pre_list, classes=[0, 1], title=f"PTK7 from Articles and Experiments\nacc = {acc * 100:.3f}%")
    plt.savefig(f"{sanlicy_path}/external_validation_confusion_matrix.pdf", bbox_inches="tight")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_filename', type=str, required=True, help="Path to the label CSV file")
    parser.add_argument('--sanlicy_path', type=str, required=True, help="Directory path to save logs and models")
    parser.add_argument('--external_data_path', type=str, required=True, help="Path to the external data CSV file for evaluation")
    args = parser.parse_args()
    main(args)
